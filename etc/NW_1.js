...
function solution(money, cost) {
    ...
    for (let i = 0; i < cost.length; i++) {
        if (nowMoney < cost[i]) {
            allCount.push(elevatorCount);
            elevatorCount = 0;
            nowMoney = money;
        }
        
        if (nowMoney >= cost[i]) {
            nowMoney = nowMoney - cost[i];
            elevatorCount = elevatorCount + 1;
        }
    }
    
    return Math.max.apply(null, allCount);;
}

// => 테스트 케이스만 있었으며, 테스트 케이스만 성공한 상태. 다른 오류 발생 가능성 높음.
// => 연속된 엘리베이터 만들 수 있는 횟수를 배열에 담아서 가장 큰 수를 반환한 결과 값을 정답으로 설정