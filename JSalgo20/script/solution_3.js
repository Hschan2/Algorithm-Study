...
각 입력값은 이름, 체력, 정신력, 기술력, 방어력으로 주어집니다. 위와 같이 만약 2명만 지원한다면 상위 30%를 선발할 수 없기에 아무도 출정하지 않습니다. 만약 4명이 지원을 하였고 모두가 점수가 다르다면 단 한 명만 출정할 수 있어요. 상위 30%에 해당하는 기술자의 이름을 return하는 solution 함수를 완성하세요.
...
function solution(data) {
    ...
    const requiredCount = Math.ceil(totalApplicants * 0.3);
    ...
    data.sort((a, b) => {
        const sumA = a.slice(1).reduce((acc, curr) => acc + curr, 0);
        const sumB = b.slice(1).reduce((acc, curr) => acc + curr, 0);

        return sumB - sumA;
    });

    const maxScore = data[0].slice(1).reduce((acc, curr) => acc + curr, 0);
    const selected = data.filter((applicant) => {
        const sum = applicant.slice(1).reduce((acc, curr) => acc + curr, 0);
        return sum === maxScore;
    });
    ...
    return selected.map((applicant) => applicant[0]).sort((a, b) => b.localeCompare(a));
}

const input_1 = [['A', 25, 24, 11, 12], ['B', 13, 22, 16, 14]];
const input_2 = [['A', 25, 25, 25, 25], ['B', 10, 12, 13, 11], ['C', 24, 22, 23, 21], ['D', 13, 22, 16, 14]];
